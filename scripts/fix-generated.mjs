...
async function main() {
    ...
    ts = ts
        // Replace broken AllowSharedBufferSource with MLAllowSharedBufferSource
        .replace(/AllowSharedBufferSource/g, 'MLAllowSharedBufferSource')

        // convert [[#anchor]] to {@link spec_url}
        // convert [[#anchor|text]] to {@link spec_url|text}
        // WebNN spec uses different style bibliographical links than WebGPU
        .replace(/([^#])\[\[([^\[].*?)\]\]/g, (_, p1, p2) => `'${p1}{@link https://www.w3.org/TR/webnn/#biblio-${p2.toLowerCase()}}'`)
